'''
File:  Discriminator_without_Generator.py
Author: Yutong Dai (rothdyt@gmail.com)
File Created: Thursday, 2018-10-24 16:01
Last Modified: Sunday, 2018-10-25 13:10
--------------------------------------------
Desscription: Train a Discriminator(CNN) without the Generator(CNN).
'''
import torch
import torchvision
...
for epoch in range(num_epochs):
    ...
    for batch_idx, (X_train_batch, Y_train_batch) in enumerate(train_loader):
        ...
        loss.backward()
        optimizer.step()
    train_accuracy.append(float(correct)/float(total))
    ...
    with torch.no_grad():
        ...
        test_accuracy.append(float(correct)/float(total))
    print('test => epoch[{}|{}]: accuracy is {}'.format(epoch+1, num_epochs, float(correct)/float(total)))
print('train accuracy is {}'.format(np.mean(train_accuracy)))
print('test accuracy is {}'.format(np.mean(test_accuracy)))
