...
private:
  ...
  /**
     ...
     */
  /*
     * Helper function for recursively building a kdtree
     */
  KDTreeNode *kdtreeBuilder(vector<Point<Dim>> &points, int curBegin, int curEnd, int curDim);

  /*
     * Helper function for partition (one step in quickselect)
     */
  int partition(vector<Point<Dim>> &points, int curBegin, int curEnd, int pivotIndex, int curDim);

  /*
     * Helper function for quick select
     */
  Point<Dim> quickSelect(vector<Point<Dim>> &points, int curBegin, int curEnd, int target, int curDim);

  /*
     * Helper function for recursively deleting all KDTreeNodes
     * Used in the destructor
     */
  void kdtreeDelete(KDTreeNode *curNode);

  /*
     * Helper function for recursively finding the nearest neighbor
     */
