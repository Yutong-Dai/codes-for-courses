...
bool isBalanced(queue<char> input)
{
    ...
    {
        if (brackets.top() == ']') // bracket is a stack! so last one is at the top!!
        {
            indicator += 1;
        }
        else
...
void scramble(queue<T> &q)
{
    stack<T> s;
    int count = 1;
    int sizeTotal = q.size();
    bool indicator = true;
    while (sizeTotal > 0)
    {
        if (count % 2 == 1)
        {
            for (unsigned i = 1; i <= fmin(count, sizeTotal); i++)
            {
                q.push(q.front());
                q.pop();
            }
...
bool verifySame(stack<T> &s, queue<T> &q)
{
    ...
    else
    {
        ...
        s.push(s_current);
        q.push(q_current);
        retval = retval && (s_current == q_current);
        return retval;
    }
}
