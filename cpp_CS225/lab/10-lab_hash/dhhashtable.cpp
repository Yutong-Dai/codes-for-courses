...
void DHHashTable<K, V>::insert(K const &key, V const &value)
{
    ...
    size_t h2 = secondary_hash(key, size);
    size_t idx = h1;
    // begin from h1, find the first NULL to insert
    for (; table[idx] != NULL; idx = (idx + h2) % size)
    {
    }
...
void DHHashTable<K, V>::remove(K const &key)
{
    ...
    if (idx != -1)
    {
        // the key exists, should remove it
        elems--;
        delete table[idx];
        table[idx] = NULL;
    }
...
int DHHashTable<K, V>::findIndex(const K &key) const
{
    ...
    size_t h1 = hash(key, size);
    size_t h2 = secondary_hash(key, size);
    for (size_t idx = h1; should_probe[idx] == true; idx = (idx + h2) % size)
    {
...
void DHHashTable<K, V>::resizeTable()
{
    ...
    for (size_t slot = 0; slot < size; slot++)
    {
        if (table[slot] != NULL)
        {
            ...
            size_t stepSize = hashes::secondary_hash(table[slot]->first, newSize);
            size_t i = 0;
            size_t idx = h;
            while (temp[idx] != NULL)
            {
                ++i;
                idx = (h + stepSize * i) % newSize;
            }
            temp[idx] = table[slot];
            should_probe[idx] = true;
        }
