...
void BTree<K, V>::split_child(BTreeNode *parent, size_t child_idx)
{
    ...
    // assign(begin, end): inlcude begin exclude end

    //order matters!!
    // modifiy the new_left first will influence the new_right as new_left point to child.
    ...
    // this calls vector built-in insert
...
void BTree<K, V>::insert(BTreeNode *subroot, const DataPair &pair)
{
    ...
    // if first_larger_idx is a valid index and the key there is the key we want to insert,
    // do nothing, we are done
    if (first_larger_idx < (subroot->elements).size() && (subroot->elements)[first_larger_idx] == pair)
    {
        return;
    }
    // sanity check after the insertion is done
    if (subroot->is_leaf)
    {
    ...
    // Check if the child becomes too large
