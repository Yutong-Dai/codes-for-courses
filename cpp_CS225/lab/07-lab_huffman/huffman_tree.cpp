                            ...
                            queue<TreeNode *> &mergeQueue)
{
     ...
     */
    TreeNode *to_be_remove = NULL;
    if (singleQueue.empty() and mergeQueue.empty())
    {
        return to_be_remove;
    }
    ...
    if (!singleQueue.empty() and mergeQueue.empty())
    {
        to_be_remove = singleQueue.front();
        singleQueue.pop();
        return to_be_remove;
    }
...
void HuffmanTree::buildTree(const vector<Frequency> &frequencies)
{
    ...
    // for (const Frequency &frequency : frequencies)
    // {
    ...
    while (singleQueue.size() + mergeQueue.size() > 1)
    {
        ...
        int subRootFrequency = (tempLeft->freq).getFrequency() + (tempRight->freq).getFrequency();
        TreeNode *internalSubRoot = new TreeNode(subRootFrequency);
        internalSubRoot->left = tempLeft;
        internalSubRoot->right = tempRight;
        mergeQueue.push(internalSubRoot);
    }
...
void HuffmanTree::decode(stringstream &ss, BinaryFileReader &bfile)
{
    ...
    while (bfile.hasBits())
    {
         ...
         */
        bool direction = bfile.getNextBit();
...
HuffmanTree::TreeNode *HuffmanTree::readTree(BinaryFileReader &bfile)
{
    ...
    if (bfile.hasBits())
    {
        ...
        if (isleaf)
        {
            Frequency leaf(bfile.getNextByte(), 0);
